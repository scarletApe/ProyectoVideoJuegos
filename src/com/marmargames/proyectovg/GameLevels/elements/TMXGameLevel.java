...
public class TMXGameLevel extends ManagedGameScene {
	...
	public void onLoadScene() {
		...
		mBitmapTextureAtlas = new BitmapTextureAtlas(
				ResourceManager.getInstance().engine.getTextureManager(), 72,
				128, TextureOptions.DEFAULT);
		mPlayerTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(mBitmapTextureAtlas,
						ResourceManager.getInstance().context, "player.png", 0,
						0, 3, 4);
		...
		try {
			final TMXLoader tmxLoader = new TMXLoader(
					ResourceManager.getInstance().context.getAssets(),
		...
		ResourceManager.getInstance().camera.setBounds(0, 0,
				tmxLayer.getHeight(), tmxLayer.getWidth());
		 ...
		 * camera.
		 */
		final float centerX = (800 - mPlayerTextureRegion.getWidth()) / 2;
		final float centerY = (480 - mPlayerTextureRegion.getHeight()) / 2;

		/* Create the sprite and add it to the scene. */
		final AnimatedSprite player = new AnimatedSprite(centerX, centerY,
				mPlayerTextureRegion, engine.getVertexBufferObjectManager());

		// Set the camera to chase the player
		ResourceManager.getInstance().camera.setChaseEntity(player);
