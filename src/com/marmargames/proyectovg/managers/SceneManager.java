...
import com.marmargames.proyectovg.layers.ManagedLayer;
import com.marmargames.proyectovg.layers.OptionsLayer;
import com.marmargames.proyectovg.menus.MainMenu;
import com.marmargames.proyectovg.menus.ManagedScene;
...
public class SceneManager extends Object {
	...
	// VARIABLES
	// ====================================================
	// These variables reference the current scene and next scene when switching
	// scenes.
	...
	// Used by the mLoadingScreenHandler, this variable ensures that the loading
	// screen is shown for one frame prior to loading resources.
	private int mNumFramesPassed = -1;
	// Keeps the mLoadingScreenHandler from being registered with the engine if
	// it has already been registered.
	private boolean mLoadingScreenHandlerRegistered = false;
	// An update handler that shows the loading screen of mNextScene before
	// calling it to load.
	private IUpdateHandler mLoadingScreenHandler = new IUpdateHandler() {
		...
		public void onUpdate(float pSecondsElapsed) {
			...
			mNumFramesPassed++;
			// And increment the amount of time that the loading screen has been
			// visible.
			...
			// On the first frame AFTER the scene has been completely loaded and
			// the loading screen has been shown for its minimum limit.
			if (mNumFramesPassed > 1
				...
				// Reset the handler & loading screen variables to be ready for
				// another use.
	...
	// PUBLIC METHODS
	// ====================================================
	// Initiates the process of switching the current managed scene for a new
	// managed scene.
	public void showScene(final ManagedScene pManagedScene) {
		// Reset the camera. This is automatically overridden by any calls to
		// alter the camera from within a managed scene's onShowScene() method.
		...
		if (pManagedScene.hasLoadingScreen) {
			...
			pManagedScene.setChildScene(
					pManagedScene.onLoadingScreenLoadAndShown(), true, true,
					true);
			// This if/else block assures that the LoadingScreen Update Handler
			// is only registered if necessary.
			...
			// Set pManagedScene to mNextScene which is used by the loading
			// screen update handler.
			mNextScene = pManagedScene;
			// Set the new scene as the engine's scene.
			mEngine.setScene(pManagedScene);
			// Exit the method and let the LoadingScreen Update Handler finish
			// the switching.
			return;
		}
		// If the new managed scene does not have a loading screen.
		// Set pManagedScene to mNextScene and apply the new scene to the
		// engine.
		mNextScene = pManagedScene;
	...
	public void showLayer(final ManagedLayer pLayer,
		...
		if (pSuspendSceneDrawing || pSuspendSceneUpdates
				|| pSuspendSceneTouchEvents) {
			// Apply the managed layer directly to the Camera's HUD
			mEngine.getCamera()
					.getHUD()
					.setChildScene(pLayer, pSuspendSceneDrawing,
			...
			mCurrentScene.setChildScene(mPlaceholderModalScene,
